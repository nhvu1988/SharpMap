...
namespace WPFSamples
{
	...
	public partial class MainWindow
	{
		...
		private void AddShapeLayer_OnClick(object sender, RoutedEventArgs e)
		{
			...
			if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
			{
				...
				if (ds.CoordinateSystem != null)
				{
					...
					lay.ReverseCoordinateTransformation =
						fact.CreateFromCoordinateSystems(ProjNet.CoordinateSystems.ProjectedCoordinateSystem.WebMercator,
							ds.CoordinateSystem);
				}
		...
		private void AddShapeLayer1_OnClick(object sender, RoutedEventArgs e)
		{
			var ofd = new OpenFileDialog {Filter = @"Shapefiles (*.shp)|*.shp"};
			if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
			{
				var layer = new GdalTileIndexRasterLayer("offlinemap", ofd.FileName, "location");
				WpfMap.MapLayers.Add(layer);
				WpfMap.ZoomToEnvelope(new Envelope(120000.00003999996, 1002000.00004, 5900000.0000399994, 6512500.0000399994));
			}
		}

		private void SelectTifFile_OnClick(object sender, RoutedEventArgs e)
		{
			var ofd = new OpenFileDialog { Filter = @"Tiffile (*.tif)|*.tif" };
			if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
			{
				var layer = new GdalRasterLayer("offlinemap", ofd.FileName);
				WpfMap.MapLayers.Add(layer);
				WpfMap.ZoomToExtents();
			}
		}
